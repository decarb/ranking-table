...
import ranking.algebra.RankingCalculator
import ranking.domain.{GameResult, RankedEntry, TeamName}

final class LiveRankingCalculator[F[_]: Applicative] extends RankingCalculator[F]:
  ...
  private def computeStandings(results: List[GameResult]): List[RankedEntry] =
    ...
    val sorted = pointsMap.toList
      .sortBy { case (team, pts) => (-pts, team.value) }
...
object LiveRankingCalculator:
  def make[F[_]: Applicative]: RankingCalculator[F] = new LiveRankingCalculator[F]
